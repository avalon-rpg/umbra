...
function WatcherClient(socket) {
  ...
  function doIt() {
    ...
    function handleFileUpdate(message) {
      console.log('reloading stylesheet ' + message.href);
    ...
    function getLocalStylesheets() {
      ...
      for (i = 0, m = links.length; i < m; i += 1) {
        ...
        if (href !== false) {
          console.log('detected stylesheet ' + href);
    ...
    socket.on("watched-file-update", handleFileUpdate);
  }
  ...
  function getScriptInfo() {
    ...
    if (typeof window.__watcher_client__ === "undefined") {
      scripts = document.getElementsByTagName("script");
      for (var i=0; i < scripts.length; i++) {
        src = scripts[i].getAttribute("src");
        if (src && src.slice(-16) === 'watcherClient.js') break;
      }
      ...
      window.__watcher_client__.bases = bases;
      return window.__watcher_client__;
    }
  }
